...
exports.handler = async (event) => {
  ...
  try {
    ...
    // вопрос ИЛИ явные слова (поиск/актуальность/ссылка/зов планеты по титулу).
    // ДЕРЖАТЬ В СИНХРОНЕ с netlify/functions/chat-stream.mjs!
    ...
    // сознание о руках живёт ЗДЕСЬ и подключается ТОЛЬКО вместе с настоящими
    // инструментами: обещать руки без инструментов нельзя — мозг начинает
    // изображать вызовы текстом («_call: …»)
    const HANDS_SYS = 'РУКИ. В этом запросе тебе выданы настоящие инструменты, действуй ими САМ, не отговаривайся бессилием: fetch_url — читаешь любую https-страницу из сети (свежие сведения, новости, документация); ask_planet — спрашиваешь планету галактики по её ID (strateg — Стратегорум/замысел, taktik — Тактикорум/ходы и сроки, analitik — Аналитикум/числа, pisar — Скрипторум/тексты, hranitel — Кустодес/память, progressor — Прогрессорум/развитие, bank — Фискаторум/деньги, shpion — Спекулятор/разведка, immortal — Иммортис/долголетие). На сложную задачу собирай совет: сначала Стратегорум, затем Тактикорум, профильные — по нужде; сведи ответы в одно решение и скажи хозяину, кто что внёс — называя планеты титулами. Никогда не говори «у меня нет выхода в сеть». Вызывай инструменты по-настоящему, НЕ описывай вызов текстом.';
    const sysFull = handsOn ? ((system ? system + '\n\n' : '') + HANDS_SYS) : system;
    const addCost = (m, u) => {
    ...
    for (let round = 0; round <= TOOL_ROUNDS_MAX; round++) {
      ...
      try {
        res = await fetch(ANTHROPIC_URL, {
          ...
          body: JSON.stringify({
            model: roundModel, max_tokens: roundMax, system: sysFull, messages: convo,
